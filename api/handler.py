from fastapi import APIRouter, UploadFile, File, WebSocket, Body
...
WORK_DIR_BASE = os.getenv('ITINERARY_WORK_DIR', '/a0/usr/workdir/itinerary_processor')
...
async def itinerary_process(file: UploadFile = File(...), num_drivers: int = 1):
    ...
    try:
        ...
        work_dir = f"{WORK_DIR_BASE}/{process_id}"
...
async def get_driver_route(driver_id: int):
    ...
    try:
        dirs = glob.glob(f"{WORK_DIR_BASE}/*")
        if not dirs:
...
async def post_driver_location(data: dict = Body(...)):
